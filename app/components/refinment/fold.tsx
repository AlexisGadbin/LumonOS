...
const Fold = (props: Props) => {
    ...
    const styles = StyleSheet.create({
        ...
        tab: {
            position: 'absolute',
            top: -18,
            left: flipped ? undefined : (number * width / 4) - (width / 4),
            right: flipped ? (number * width / 4) - (width / 4) : undefined,
            zIndex: 0,
        },
        name: {
            fontFamily: 'VT323',
    ...
    return (
      <View style={[styles.fold, flipped && { 
          transform: [
            { translateY: 20 }, // Add space below the card
            { rotateZ: '180deg' },
            { translateY: -20 } // Move it back after rotation
          ] 
        }]}>
